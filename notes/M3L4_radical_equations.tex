...
\begin{itemize}
    \item To solve a radical equation: isolate a radical, raise both sides to a power matching the index, repeat if another radical remains, then solve.
    \item Raising both sides to a power can introduce \textbf{extraneous solutions} --- always check every solution in the \textbf{original} equation and discard any that fail.
    \item Radical equations model real formulas, such as the skid-mark speed formula $v=k\sqrt{d}$.
\end{itemize}
\end{friendlyrule}
