...
\[
\text{domain}=\{1,2,5,7\},\qquad \text{range}=\{4,6,-3,10\}.
\]
...
\emph{Note:} it is perfectly fine for one $y$-value to be paired with more than one $x$-value.
\end{solutionbox}
...
\[
\text{Player 1}\to 6'2'',\qquad \text{Player 2}\to 6'5'',\qquad \text{Player 3}\to 6'8'',\qquad \text{Player 4}\to 6'4''.
\]
...
\begin{itemize}
    \item[(a)] An upward-opening parabola (a ``$U$'' shape).
    \item[(b)] A sideways-opening parabola (a ``$\subset$'' shape, opening left).
    \item[(c)] A wave-like curve (one $y$-value for every $x$).
    \item[(d)] A circle.
\end{itemize}
\end{friendlyex}
...
has name $f$. Its rule says: ``to find the number paired with $x$, square $x$ and add $1$.''
...
\section*{6. More Practice with Function Notation}
...
\begin{itemize}
    ...
    \item A relation is a function if every $x$-value in the domain corresponds to exactly one $y$-value.
