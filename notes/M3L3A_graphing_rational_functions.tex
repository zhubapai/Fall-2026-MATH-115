...
\begin{itemize}
    \item The toolkit functions $\frac1x$ and $\frac{1}{x^2}$ both have vertical asymptote $x=0$ and horizontal asymptote $y=0$; $\frac1x$ is odd (Quadrants I, III), while $\frac{1}{x^2}$ is even (Quadrants I, II).
    \item Writing a rational function as $\frac{1}{x-h}+k$ (or $\frac{1}{(x-h)^2}+k$) shifts the toolkit graph so the vertical asymptote is $x=h$ and the horizontal asymptote is $y=k$.
    \item A full graph combines: domain, holes, vertical/horizontal asymptotes, $x$- and $y$-intercepts, and a table of additional points to confirm the shape of each branch.
\end{itemize}
\end{friendlyrule}
